...
import com.itz.mahalla.use_cases.subscription_module.subscription_entry.dao.*;
...
public class SubscriptionEntryController {
	...
	@Autowired
	SubscriptionEntryService ser;
	@Autowired
	ClaimsSet claimsSet;

	@PostMapping(value ="/subscription-entry")
	public ResponseEntity<?> masterSet(HttpServletRequest request,@RequestBody SubscriptionEntryDao getVal) throws Exception {
		ClaimsDao claimsDao = claimsSet.getClaimsDetailsAfterSet(request.getHeader("Authorization"));
		if(ser.getPk2(getVal.getName())!=null)
			throw new Exception(getVal.getName()+" Value Already Set");
		ser.setData(new ModelMapper().map(getVal,SubscriptionEntry.class));
		return new ResponseEntity<>(new ReportDao("Added Successfully",true), HttpStatus.OK);
	}

	@PostMapping(value ="/subscription-entry/add-all")
	public ResponseEntity<?> masterSetAddAll(HttpServletRequest request, @RequestBody List<SubscriptionEntryDao> getVal) throws Exception {
		ClaimsDao claimsDao = claimsSet.getClaimsDetailsAfterSet(request.getHeader("Authorization"));
		ser.setAllData(new ModelMapper().map(getVal, new TypeToken<List<SubscriptionEntry>>(){}.getType()));
		return new ResponseEntity<>(new ReportDao("Imported Successfully", true), HttpStatus.OK);
	}

	@PostMapping(value ="/subscription-entry/delete-all")
	public ResponseEntity<?> masterSetDeleteAll(HttpServletRequest request, @RequestBody List<SubscriptionEntryDao> getVal) throws Exception {
		ClaimsDao claimsDao = claimsSet.getClaimsDetailsAfterSet(request.getHeader("Authorization"));
		ser.delAllData(new ModelMapper().map(getVal, new TypeToken<List<SubscriptionEntry>>(){}.getType()));
		return new ResponseEntity<>(new ReportDao("Deleted Successfully", true), HttpStatus.OK);
	}

	@PutMapping(value ="/subscription-entry")
	public ResponseEntity<?> masterUpdate(HttpServletRequest request,@RequestBody SubscriptionEntryDao getVal) throws Exception {
		ClaimsDao claimsDao = claimsSet.getClaimsDetailsAfterSet(request.getHeader("Authorization"));
		SubscriptionEntry setVal = ser.getPk1(getVal.getId());
		if(setVal!=null){
	...
	@GetMapping(value ="/subscription-entry/{id}")
	public ResponseEntity<?> masterGet(HttpServletRequest request
		...
		return new ResponseEntity<>(new ResultDao( new ModelMapper().map(getVal, SubscriptionEntryDao.class),"Fetched Successfully",true), HttpStatus.OK);
	}

	@GetMapping(value ="/subscription-entry")
	public ResponseEntity<?> masterGetAll(HttpServletRequest request
		...
		ClaimsDao claimsDao = claimsSet.getClaimsDetailsAfterSet(request.getHeader("Authorization"));
		List<SubscriptionEntry> getVal;
		long totalCount;
		if(isPagination){
			Page<SubscriptionEntry> getAllWtPg = ser.getAllDataByPg(pageNumber-1,pageSize-(pageNumber-1),searchKey);
			...
			return new ResponseEntity<>(new ResultsDao(new ModelMapper().map(getVal,
					new TypeToken<List<SubscriptionEntryDao>>() {
